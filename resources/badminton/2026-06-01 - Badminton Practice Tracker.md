---
type: resource
---

# Badminton Practice Tracker

Related area: [[areas/badminton/badminton|Badminton]]

Use this to track badminton practice without turning every session into a project. Create a dated session note only when the session has useful feedback, drills, or decisions worth keeping.

## Capture Rule

- Record every session as one row in the Session Log.
- Normal self practice: update only this tracker.
- Important self practice: update this tracker and create a dated session note.
- Coach practice: update this tracker and create a dated session note.
- Match or game review: update this tracker and create a dated session note.
- Store detailed session notes in `resources/badminton` using `YYYY-MM-DD - Badminton Coach Practice.md` or `YYYY-MM-DD - Badminton Self Practice.md`.

## Current Focus

-  [[2026-05-25 - Badminton Fundamentals and Doubles Positioning]]

## Weekly Tracker

| Week | Self Practice | Coach Practice | Total Hours | Main Focus | Best Improvement | Main Weakness | Next Focus |
|---|---:|---:|---:|---|---|---|---|
| 2026-W22 | 0 | 1 | 2 | Footwork |  | Redundant steps; weak net play and rear-corner movement for smash preparation | 4-corner footwork and resistance band endurance |
| 2026-W23 | 0 | 0 | 0 |  |  |  |  |

## Skill Tracker

| Skill | Current Level | Evidence | Next Drill |
|---|---|---|---|
| Footwork | Needs work | Coach feedback: [[resources/badminton/2026-05-31 - Badminton Coach Practice]]; redundant steps; weak net play and rear-corner movement | 4-corner footwork drill |
| Serve |  |  |  |
| Clear |  |  |  |
| Drop |  |  |  |
| Smash |  |  |  |
| Drive |  |  |  |
| Net play |  |  |  |
| Doubles positioning |  |  |  |

## Session Log

| Date | Type | Duration | Focus | Result | Follow-up |
|---|---|---:|---|---|---|
| 2026-05-31 | Coach practice | 2h | Footwork for net play and both rear corners | [[resources/badminton/2026-05-31 - Badminton Coach Practice|Many redundant steps; smash preparation footwork needs work]] | 4-corner footwork and resistance band endurance |
|  | Self practice |  |  |  |  |
|  | Coach practice |  |  |  |  |

## Coach Practice Template

```md
# Badminton Coach Practice - YYYY-MM-DD

Related area: [[areas/badminton/badminton|Badminton]]
Tracker: [[resources/badminton/2026-06-01 - Badminton Practice Tracker|Badminton Practice Tracker]]

## Session

Date:
Coach:
Duration:
Location:

## Coach Feedback

- 

## Corrections

- 

## Assigned Drills

- 

## Self Practice Follow-up

- 
```
