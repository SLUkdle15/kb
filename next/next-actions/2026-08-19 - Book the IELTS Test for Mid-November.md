...
- [ ] Compare IDP and British Council for date, centre location, and fee.
- [ ] Check the Speaking slot. It may fall on a different day from the other three sections; both dates have to work.
- [ ] Confirm the ID document. The passport or ID used at registration must be the same one carried on test day, and the name must match exactly.
- [ ] Pay and get the confirmation.
...
Study first, book when ready — but the slack is gone. 2026-09-21 was the backstop, and it is here: September is the last month a booking still lands the test in mid-November, so this is now a this-week action rather than a dated one.
...
Centre choice used to be the thing that eroded, on the assumption that One Skill Retake was centre-dependent. It is not, in Vietnam — see the checklist above.
