# Fields Near Me

Soccer fields within reach of home, for when the current rental ends and the [[next/waiting/2026-08-17 - Soccer Field Deposit of 2M VND|2M VND deposit]] comes back. Capture names as they come up, even bare ones — a street or venue name is a lead to confirm, not a booking.

Same handling as [[areas/badminton/courts-near-me|Courts Near Me]]: anything without an address stays marked unconfirmed.

## Fields

| Field | Location | Pitch | Confirmed |
| --- | --- | --- | --- |
| Sân bóng Thanh Lâm | — | — | name only |
| Sân bóng Mễ Trì Thượng | — | — | name only |
| Sân bóng ĐH Sư Phạm | — | — | name only |
| Sân bóng Nguyễn Dương | — | — | name only |
| Sân bóng VYF | — | — | name only |
| Sân bóng HQV | — | — | name only |
| Sân bóng Yên Hòa | — | — | name only |
| Sân bóng Mỹ Đình 2 / MHDI | — | — | name only |

## Worth Adding Later

Only once a field is actually played on, and only what changes the choice for a weekly 17:30 [[next/calendar/2026-07-29 - Weekly Tuesday Soccer|Tuesday match]]:

- Pitch size — 5, 7, or 11-a-side, and whether it splits.
- Surface and its state: artificial turf age, bald patches, drainage after rain.
- Floodlights, since 17:30 kickoffs run into the dark for much of the year.
- Booking method, whether a weekly slot can be held, and the deposit terms.
- Price per slot, and what it becomes split across the regular squad.
- Parking, and whether there are changing rooms or just a tap.
